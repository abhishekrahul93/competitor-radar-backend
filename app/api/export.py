...
async def export_brief_pdf(report_id: int, token: str = Query(default=""), db: AsyncSession = Depends(get_db)):
    user_id = verify_token(token)
    if not user_id:
        return Response(content="Invalid or missing token", status_code=401)
    ...
    doc.build(elements)
    buf.seek(0)
    return Response(content=buf.read(), media_type="application/pdf", headers={"Content-Disposition": "attachment; filename=CompetitorRadar_" + comp_name.replace(' ','_') + ".pdf"})

@router.get("/all")
async def export_all_briefs(token: str = Query(default=""), db: AsyncSession = Depends(get_db)):
    user_id = verify_token(token)
    if not user_id:
        return Response(content="Invalid or missing token", status_code=401)
    ...
    rows = result.all()
    if not rows:
        return Response(content="No reports found", status_code=404)
